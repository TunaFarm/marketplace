    ...
    <div className="w-2/6 m-5">
      <div className="flex flex-col justify-between h-36 p-2">
        ...
        <div className="flex justify-start mt-1 mb-1">
          {itemProperties.map((item) => (
            <div className="flex items-center mr-2" key={item.id}>
      ...
      <div>
        ...
        <div className="flex border-transparent rounded-2xl px-10 py-7 bg-[#20252c]  ">
          {aboutProperties.map((item) => (
            <div key={item.id} className="mr-12">
              ...
              <div className="flex">
                {Array.isArray(item.content) ? (
                  item.content.map((text) => (
                    <p
                      key={text}
                      className="bg-[#2f3740] border-transparent rounded-2xl text-white px-4 py-1 mr-4"
                    >
                      {text}
                    </p>
                  ))
      ...
      <div>
        ...
        <div className="my-4">
          <h1 className="text-white text-2xl font-bold px-2 py-3">
            Description
          </h1>
          <p className="text-[#bdc0c2] px-2">
            Floating in the streets of Etheria, this cute vending robot sells
            drinks and snacks. There is something for everyone!
          </p>
        </div>
        <div className="flex items-center px-2 my-10">
          ...
          <div
            ...
            style={{
              WebkitMask: `url("https://www.sandbox.game/img/07_Asset_Info/twitter-icon.svg") no-repeat center`,
            }}
          />
          <div
            className="w-10 h-10 ml-5"
            style={{
              WebkitMask: `url("https://www.sandbox.game/img/07_Asset_Info/facebook-icon.svg") no-repeat center`,
            }}
          />
        </div>
        <div>
              ...
              <div className="w-full">
                <div className="grid grid-cols-2 gap-4  ">
                  {relateData.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-md border-2 border-blue-600"
                    >
                      <div className="px-4 py-4 border-box">
                        <div>
                          <p className="text-white"> {item.title}</p>
                          <p className="text-[#bdc0c2] text-xs">
                            {" "}
                            {item.rareless}
                          </p>
        ...
        <div className="my-4">
          ...
          <div className="text-[#bdc0c2] px-2">
            <p className="py-1 text-sm">
              When a creator designs an asset, they have to distribute points
              amongst the chosen attributes.
            </p>
            <p className="py-1 text-sm">
              A creator can change the amount of attributes while creating an
              asset.
            </p>
            <p className="py-1 text-sm">
              When a Game Maker update is released, new mechanics can use
              existing attributes or new ones released with the update.
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="border-transparent rounded-2xl h-[250px] w-full p-6 bg-[#20252c]">
            ...
            <div className="text-[#bdc0c2] px-2">
              ...
              <p className="py-1 text-sm">
                Assets need the required attributes to be used with a game
                mechanism
              </p>
              <p className="py-1 text-sm">
